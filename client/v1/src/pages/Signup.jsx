...
import {signupSchema} from '../../../../common/index.js'
...
const Signup = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center max-w-[500px] mx-[16px] xs:mx-[auto] my-[100px] p-4 border-[1px] shadow-lg rounded-md bg-primary'>
      <h2 className='text-2xl font-semibold text-center'>Sign Up</h2>
      <form onSubmit={handleSubmit(onSubmit)} className='px-12 [&>*]:mt-4 [&>*]:w-full [&>*]:p-[8px]  [&>*]:border-[1px]  [&>*]:rounded-md [&>p]:border-none [&>p]:m-0 [&>p]:p-0'>
        ...
        <select {...register("currency")} name="currency" value={userDetails.currency} onChange={handleUserDetails} onFocus={(e) => { e.target.size = 5}}
    onBlur={(e) => { e.target.size = 1; }} className='w-full'>
      ...
      <option value="ZWB" label="Zimbabwean bonds">ZWB</option>
        ...
        <button disabled={isSubmitting} type='submit' className='bg-secondary hover:bg-[#a3ccf7] active:bg-secondary'>{isSubmitting ? "Creating..." : "Create Account" }</button>
    </form>
    {errors.root && <p className='text-red-500 mt-2'>{errors.root.message}</p>}
    <p className='mt-2'>Already have an account?
      <Link to='/v1/login' className='underline ml-2'>Login</Link>
    </p>
    </div>
  )
}
