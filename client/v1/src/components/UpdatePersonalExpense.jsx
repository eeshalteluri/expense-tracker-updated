...
const UpdatePersonalExpense = ({ id, fetchData, fetchExpense, closeModal }) => {
  const navigate = useNavigate()
  const [newExpenseDetails, setNewExpenseDetails] = useState({
    id: id,
    expName: fetchExpense.expName,
    expAmount: fetchExpense.expAmount,
    expDate: fetchExpense.expDate,
    expDescription: fetchExpense.expDescription,
  })
    ...
    <div className='bg-primary flex flex-col padding-x padding-y rounded-lg py-4'>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <label className='flex flex-col m-1'>
          ...
          <input
            {...register("expName")}
            value={newExpenseDetails.expName}
            name="expName"
            type="text"
            className='p-1'
        ...
        <label className='flex flex-col m-1'>
          ...
          <input
            {...register("expAmount")}
            value={newExpenseDetails.expAmount}
            name="expAmount"
            type="number"
            className='p-1'
            onChange={newExpensehandler}
        ...
        <label className='flex flex-col m-1'>
          ...
          <input
            {...register("expDate")}
            value={newExpenseDetails.expDate}
            name="expDate"
            type="date"
            className='p-1'
            onChange={newExpensehandler}
        ...
        <label className='flex flex-col m-1'>
          Add any notes:
          <textarea
            {...register("expDescription")}
            value={newExpenseDetails.expDescription}
            name="expDescription"
            rows={4}
            onChange={newExpensehandler}
          />
        ...
        <button disabled={isSubmitting} type='submit' className='w-full bg-secondary hover:border-2 active:bg-[#B3E2A7] rounded mt-4'>
...
export default UpdatePersonalExpense
